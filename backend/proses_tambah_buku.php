...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($nameCount > 0) {
                echo "<script>alert('Error: Foto sudah ada di database.');location.href='../frontend/tambah_buku.php';</script>";
            } else {
                ...
                if ($stmt->execute()) {
                    echo "<script>alert('Buku Sukses Diupload');location.href='../frontend/buku.php';</script>";
                } else {
